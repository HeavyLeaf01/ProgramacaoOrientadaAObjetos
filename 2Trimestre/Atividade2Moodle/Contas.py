...
class Conta:
    ...
    @property
    def numero(self): # Java: getNumero()
        return self.__numero

    @property
    def titular(self): # Java: getTitular()
        partesNomes = self.__titular.split(" ")
        tamanho = len(partesNomes)
        return partesNomes[0] + " " + partesNomes[tamanho - 1]

    @titular.setter
    def titular(self, titular): # Java: setTitular(String titular)
        self.__titular = titular

    @saldo.setter
    def saldo(self, saldo): # Java: setSaldo(float saldo)
        if saldo < 0:
